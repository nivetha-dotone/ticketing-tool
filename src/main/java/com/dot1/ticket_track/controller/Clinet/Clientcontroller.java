...
public class Clientcontroller {
    ...
    @GetMapping("/getTotalTicketOfCME/{cmeID}")
    public  ResponseEntity<?> totalTicketCME(@PathVariable Long cmeID){
        try{
            Integer gettotaled = ticketDservice.gettotalTicketByCmeID(cmeID);
            if(gettotaled!=null){
    ...
    @GetMapping("/getTicketByCMEID/{cmeID}")
    public  ResponseEntity<?> getTicketCME(@PathVariable Long cmeID,HttpServletRequest request){
        try{
            List<mTicketSdeatils> gettotaled = ticketDservice.getTicketofCmeID(cmeID,request);
            if(gettotaled!=null){
                return new ResponseEntity<>(gettotaled,HttpStatus.OK);
            }else{
                return new ResponseEntity<>(HttpStatus.NOT_ACCEPTABLE);
    ...
    @PutMapping("/ChangeStatusByClient/{tcktID}/{gmid}")
    public  ResponseEntity<?> UpdateStatusByClient(@PathVariable Long tcktID,@PathVariable Integer gmid,HttpServletRequest request ){
        try{
    ...
    @GetMapping("/dropofStatusClient")
    public  ResponseEntity<?> dropdownofStatus(){
        try{
            List<mGeneralMaster> mGeneralMasters = ticketDservice.dropdownOFStatusClient();
            if(mGeneralMasters!=null){
                return new ResponseEntity<>(mGeneralMasters,HttpStatus.OK);
            }else{
                return new ResponseEntity<>(HttpStatus.NOT_ACCEPTABLE);
    ...
    @PostMapping(value = "/AttchMentOnHold", consumes = MediaType.MULTIPART_FORM_DATA_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> AddAttachmentHold( @RequestPart("AttachmentD") String  AttachmentD,
                                                @RequestPart(value = "files", required = false) MultipartFile[] files) {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            objectMapper.registerModule(new JavaTimeModule());
            AttachmentDto attachmentDto = objectMapper.readValue(AttachmentD, AttachmentDto.class);
            ...
            if (files != null) {
                for (MultipartFile file : files) {
                    ...
                    Attachment attachment = new Attachment();
                    String originalFilename = file.getOriginalFilename();
                    String modifiedFileName = onHoldmodifyFileName(originalFilename);
                    attachment.setId(attachmentMasteRepo.newIdAttached());
                    attachment.setFileName(modifiedFileName);
                    attachment.setFileType(file.getContentType());
                    mTicketSdeatils getticketbyid = new mTicketSdeatils();
                    getticketbyid.setTicketid(attachmentDto.getTicketID());
                    attachment.setTicketDt(getticketbyid);

                    try {
                        // Save file on disk and get the path
                        String filePath1 = saveModifiedFile(file,modifiedFileName);
                        attachment.setFilePath(filePath1);
                        Attachment save = attachmentMasteRepo.save(attachment);
                        save.setTicketDt(null);
    ...
    private String onHoldmodifyFileName(String originalFileName) {
        String fileExtension = originalFileName.substring(originalFileName.lastIndexOf("."));
        String fileNameWithoutExtension = getFileNameWithoutExtension(originalFileName);
