...
public class TicketDController {
    ...
    @GetMapping(value = "/getAttachment", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<Attachment>> getAllAttachments() {
        try {
            List<Attachment> attachments = attachmentMasteRepo.findAll();
...
//            }
    ...
    public ResponseEntity<?> AddAttachmentHold( @RequestPart("tickets") String  AttachmentD,
                                                         @RequestPart(value = "files", required = false) MultipartFile[] files) {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            objectMapper.registerModule(new JavaTimeModule());
            ...
            if (files != null) {
                for (MultipartFile file : files) {
                    ...
                    Attachment attachment = new Attachment();
                    String originalFilename = file.getOriginalFilename();
                    String modifiedFileName = modifyFileName(originalFilename, attachmentDto.getTicketCode());
                    attachment.setId(attachmentMasteRepo.newIdAttached());
                    attachment.setFileName(modifiedFileName);
                    attachment.setFileType(file.getContentType());
                    mTicketSdeatils getticketbyid = ticketDservice.getticketbyid(attachmentDto.getTicketID());
